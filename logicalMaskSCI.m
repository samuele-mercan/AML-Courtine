...
for i = 1:numel(gaitCycles)
    ...
    allFO_left = [allFO_left, (i-1) + data.(GC).FO_left];
    allFS_left = [allFS_left, (i-1) + data.(GC).FS_left];
    allFO_right = [allFO_right, (i-1) + data.(GC).FO_right];
    allFS_right = [allFS_right, (i-1) + data.(GC).FS_right];
end
