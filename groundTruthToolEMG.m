...
figure()
hold on
title(strcat(dataName, "_T01_left-LMG"))
plot(data.(fields{1}).T_01.Raw.EMG.LMG);
fsEMG = data.(fields{1}).T_01.fsEMG;
[x, y] = ginput;
T01_Onset_LMG_left = x/fsEMG;
[x, y] = ginput;
T01_FO_left = x/fsEMG;
...
plot(data.(fields{1}).T_01.Raw.EMG.RMG);

[x, y] = ginput;
T01_FS_right = x/fsEMG;
[x, y] = ginput;
T01_FO_right = x/fsEMG;

figure()
hold on
title(strcat(dataName, "_T02_left"))
plot(data.(fields{1}).T_02.Raw.EMG.LTA);
fsKin = data.(fields{1}).T_02.fsEMG;
[x, y] = ginput;
T02_FS_left = x/fsKin;
[x, y] = ginput;
T02_FO_left = x/fsKin;
