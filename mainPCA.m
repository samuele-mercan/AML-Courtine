...
figure('Name', 'Cumulated variance wrt right food')
plot(1:length(cumsum(explained)), cumsum(explained))
hold on
%95% line
line([1,length(cumsum(explained))], [0.95, 0.95])
...
figure('Name', 'Top10 features contributing to first 3 PCs wrt right foot')
bar(1:10, top10_features)
ylabel('Loading of feature')

%add code for anova comparison using p = anova1(featuredata)